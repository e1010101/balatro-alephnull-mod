...
extern MY_HIGHP_OR_MEDIUMP vec2 entity;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    // reality "breathing" — the sprite ripples radially like it can't hold its shape
    number warp = 0.010*sin(7.0*rad - 2.2*T + seed) + 0.006*sin(5.0*ang + 1.6*T);
    ...
    number ab_amt = 0.006 + 0.014*rad + 0.012*burst + 0.003*sin(2.4*T + entity.x);
    vec2 ab_dir = vec2(cos(0.8*T + entity.x*0.5), sin(0.8*T + entity.x*0.5));
    ...
    tex.rgb *= (0.96 + 0.04*sin(uv.y*90.0 + T*8.0)) * (1.0 + 0.08*sin(2.0*T + rad*6.0));
